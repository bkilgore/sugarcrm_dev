...
include_once('include/pdf/class.pdf.php');
...
var $ezPageCount=0;
var $lastWidth; //this keeps track of the last width of an object (table/image);
// ------------------------------------------------------------------------------
...
function Cezpdf($paper='a4',$orientation='portrait'){
	...
	if (!is_array($paper)){
		switch (strtoupper($paper)){
			...
			case 'TABLOID': {$size = array(0,0,792.00,1224.00); break;} // cn: bug 3627 - added Tabloid size
		}
...
function ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$gap,$size,&$y,$optionsAll=array(), $startX0 = 0, $startX1=0){
...
//NEW CODE
if(!empty($optionsAll['shadeHeadings'])){
				$shadeHeaderObject = $this->openObject();
				$this->closeObject();
				$this->addObject($shadeHeaderObject);
				$this->reopenObject($shadeHeaderObject);
				$startHeight = $y;
				$this->setColor($optionsAll['headTextCol'][0], $optionsAll['headTextCol'][1], $optionsAll['headTextCol'][2], 1);
}	
		//END NEW CODE
...
if(!empty($optionsAll['shadeHeadings'])){
			$this->closeObject();
			$headingHeight = $mx + $gap * 2 - $decender;
			$this->setColor($optionsAll['headCol'][0],$optionsAll['headCol'][1],$optionsAll['headCol'][2],1);
        	$this->filledRectangle($startX0 - $gap/2 ,$y-2,$startX1 - $startX0 - 1, -1 * $mx);
}		
//END NEW 
...
function getDisplayCount($cnt){
	 $kCount = floor(($cnt ) / 1000);
     $dispCount = $cnt - 1000 * $kCount;
     if($kCount > 0){
     	$kCount .='K';
     }else{
     	$kCount = '';
     }
     return $kCount . $dispCount;	
}
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  // 'showHeadings' => 0 or 1
  // 'shaded'=> 0,1,2,3 default is 1 (1->alternate lines are shaded, 0->no shading, 2-> both shaded,4-> shade columns alternating second uses shadeCol2)
  ...
  $defaults = array(
   'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>12
    ...
    ,'innerLineThickness'=>1,'outerLineThickness'=>1,'splitRows'=>0,'protectRows'=>1, 'shadeHeadings'=>0, 'headCol'=>array(.7,.7,.7), 'headTextCol'=>array(0,0,0), 'display_footer'=>0, 'footer'=>array('SugarCRM','http://www.sugarcrm.com'),
    'display_header'=>0, 'footer'=>'SugarCRM Report','showRowCount'=>0,
    );
  ...
  //edit the left margin if the showing of row numbers will cause bad things
  ...
  if ($options['showHeadings']){
     ...
     $this->setColor($options['headTextCol'][0],$options['headTextCol'][1],$options['headTextCol'][2],1);
   $dm = $this->ez['leftMargin']-$baseLeftMargin;
    foreach($basePos as $k=>$v){
      $pos[$k]=$v+$dm;
    }
    ...
    $headingHeight = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['fontSize'],$y,$options, $x0, $x1);
    $y0 = $y+$headingHeight;
    $y1 = $y;
...
*/
  ...
  if ($options['shaded'] && $options['shaded'] != 4 ){
    $this->saveState();
  ...
  if($options['display_header'] && isset($options['header'])){
		$header = $options['header'];
		$headerX = ($this->ez['pageWidth'] - $this->getTextWidth( $options['fontSize'], $header) )/2 ;
		$headerY = ($this->ez['pageHeight'] - ($this->getFontHeight($options['fontSize']) * 3));
		$this->addText($headerX , $headerY, $options['fontSize'] , $header);
   }
        if($options['display_footer'] && isset($options['footer'])){
        	$footer = $options['footer'];
        	$footerX = ($this->ez['pageWidth'] - $this->getTextWidth( $options['fontSize'], $footer[0]) )/2 ;
        	$footerY = $this->getFontHeight($options['fontSize']);
        	$this->addText($footerX , $footerY, $options['fontSize'] , $footer[0]);
        	if(!empty($footer[1])){
        		$this->addLink($footer[1],$footerX, $footerY ,  $footerX +  $this->getTextWidth( $options['fontSize'], $footer[0]), 2 * $footerY);
...
//        $x1=$x1+$dm;
      ...
      foreach($cols as $colName=>$colTitle){
      	if ($options['shaded']== 4 && $colIndex % 2 == 0 ){
          $this->saveState();
          $textObjectId = $this->openObject();
          $this->closeObject();
          $this->addObject($textObjectId);
  ...
  //            $this->ezNewPage();
           ...
           if ($options['shaded'] == 4 && $colIndex%2==0){
        	 ...
        	 $this->filledRectangle($pos[$colName],$this->y - 1,$maxWidth[$colName],$options['fontSize'] );
             $this->restoreState();
      }
          $colIndex++;
        }
      ...
      if ($options['shaded'] && $options['shaded'] != 4 && $cnt%2==0){
        $this->closeObject();
        $this->setColor($options['shadeCol'][0],$options['shadeCol'][1],$options['shadeCol'][2],1);
    ...
    if ($options['splitRows']==0){
      if ( ( ($this->ezPageCount != $pageStart) || (isset($this->ez['columns']) && $this->ez['columns']['on']==1 && $columnStart != $this->ez['columns']['colNum'] ))  && $secondTurn==0){
        ...
        $x1=$baseX1+$dm;
      	...
      	if($options['showRowCount'] && $cnt > 0){
      		$dispCount = $cnt;
     		$this->addText($this->ez['leftMargin']  - $this->getTextWidth($options['fontSize'] - 2, $dispCount) -10, $this->y, $options['fontSize'] - 2, $dispCount);
	}
        $this->transaction('commit');
        $ok=1;
      }
